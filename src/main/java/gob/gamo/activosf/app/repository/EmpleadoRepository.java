...
public interface EmpleadoRepository extends JpaRepository<OrgEmpleado, Integer> {
    ...
    @Query("Select e from OrgEmpleado e, OrgPersona p, OrgUnidad u where e.idPersona = p.idPersona " +
            "and p.idPersona = :idPersona " +
            "and e.idUnidad = u.idUnidad " +
    ...
    Optional<OrgEmpleado> findByIdPersonaActivo(@Param(value = "idPersona") Integer idPersona,
    ...
    List<OrgEmpleado> findByAllIdPersonaCargo(@Param(value = "idPersona") Integer idPersona,
            @Param(value = "idCargo") Integer idCargo, @Param(value = "fecha") Date fecha);

    @Query("Select e from OrgEmpleado e, OrgUnidad p where e.idUnidad = p.idUnidad "
            + " and p.idUnidad = :id  ") 
    List<OrgEmpleado> empleadosUnidad(@Param(value = "id") Integer id);

    @Query("Select e from OrgEmpleado e, OrgUnidad p where e.idUnidad = p.idUnidad "
            + " and p.idUnidad = :id and e.fechaBaja is null ")  
    List<OrgEmpleado> empleadosUnidadAct(@Param(value = "id") Integer id);

    @Query("Select e from OrgEmpleado e, OrgUnidad p where e.idUnidad = p.idUnidad "
            + " and p.idUnidad = :id and e.rolempleado = :rolempleado and e.fechaBaja is null ")
    List<OrgEmpleado> empleadosUnidad(@Param(value = "id") Integer id,
            @Param(value = "rolempleado") Integer rolempleado);

    @Query("Select e from OrgEmpleado e, OrgUnidad p where e.idUnidad = p.idUnidad " +
            " and p.idUnidad = :id and e.fechaBaja is null " +
            " and e.rolempleado = p.rolempleado ")
    List<OrgEmpleado> empleadosBoss(@Param(value = "id") Integer id);

    @Query("Select e from OrgEmpleado e, OrgUnidad p " +
            "where e.idUnidad = p.idUnidad " +
            "and e.fechaBaja is null " +
            "and e.rolempleado = p.rolempleado " +
            "and e.fechaIngreso = (select max(e2.fechaIngreso) from OrgEmpleado e2 " +
