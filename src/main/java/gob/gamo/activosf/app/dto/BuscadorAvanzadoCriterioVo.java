...
public class BuscadorAvanzadoCriterioVo implements Serializable {
    ...
    public HashMap<CriteriosBusquedaEnum, Object> getCriterios() {
        ...
        if (!esNulo(codigosContables)) criterios.put(CriteriosBusquedaEnum.CODIGOS_CONTABLES, codigosContables);
        if (!esNulo(familias)) criterios.put(CriteriosBusquedaEnum.FAMILIAS, familias);
        if (!esNulo(subfamilias)) criterios.put(CriteriosBusquedaEnum.SUB_FAMILIAS, subfamilias);
        if (!esNulo(centroCosto)) criterios.put(CriteriosBusquedaEnum.CENTRO_COSTO, centroCosto);
        ...
        if (!esNulo(ambientes)) criterios.put(CriteriosBusquedaEnum.AMBIENTES, ambientes);
        if (!esNulo(area)) criterios.put(CriteriosBusquedaEnum.AREAS, area);
        if (!esNulo(tipoAsignacion)) criterios.put(CriteriosBusquedaEnum.TIPO_ASIGNACION, tipoAsignacion);
        if (!esNulo(estado)) criterios.put(CriteriosBusquedaEnum.ESTADO, estado);
        ...
        if (!esNulo(fechaHistoricaInferior) && !esNulo(fechaHistoricaSuperior))
            criterios.put(
                    CriteriosBusquedaEnum.FECHA_HISTORICA, new Date[] {fechaHistoricaInferior, fechaHistoricaSuperior});
        if (!esNulo(estadoUsoActivo)) criterios.put(CriteriosBusquedaEnum.ESTADO_USO_ACTIVO, estadoUsoActivo);
        if (!esNulo(tipoActualizacion)) criterios.put(CriteriosBusquedaEnum.TIPO_ACTUALIZACION, tipoActualizacion);
        if (!esNulo(gestion)) criterios.put(CriteriosBusquedaEnum.GESTION, gestion);
        if (!esNulo(proveedor)) criterios.put(CriteriosBusquedaEnum.PROVEEDOR, proveedor);
