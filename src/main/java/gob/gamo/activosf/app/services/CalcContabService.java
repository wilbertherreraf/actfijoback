...
public class CalcContabService {
    ...
    private static final Integer DIAS_A_DEPRECIAR_DESDE_REGISTRO =
            0; // 1 en la version similiar al VSIAF, respadado con Acta 5.
    ...
    public void calcular(
            BigDecimal tipoCambioIncorporacion, BigDecimal tipoCambioCalculo, BigDecimal tipoCambioVidaUtil) {
        System.out.println(">>> CALC VO >>" + afActivoFijo.getCodigoExtendido() + " > fecha activo > " + fechaActivo
                + " > fecha Calculo >" + fechaCalculo);
        ...
        boolean fechaCalculoMayorIgualAFechaActivo =
                fechaCalculo.isAfter(fechaActivo) || fechaCalculo.isEqual(fechaActivo);
        ...
        if (fechaCalculoMayorIgualAFechaActivo && existenTiposCambio) {
             ...
             */
            boolean casoEspecialSobrante = "MIGRAC".equals(afActivoFijo.getCatTipoActualizacion())
                    && BigDecimal.ZERO.compareTo(afActivoFijo.getCostoActual()) == 0;
            ...
            if (deprecia) {
                ...
                actualizacionDepreciacionAcumulada = depreciacionAcumuladaInicial
                        .multiply(factorActual, MATH_CONTEXT)
                        .subtract(depreciacionAcumuladaInicial, MATH_CONTEXT);
            } else {
            ...
            String mensaje = "Error con los parametros de entrada [" + afActivoFijo.getCodigoExtendido()
                    + "|Activo:" + fechaActivo
                    + "|Calculo:" + fechaCalculo + "] : Fecha de Calculo Mayor o Igual a Fecha del Activo("
                    + fechaCalculoMayorIgualAFechaActivo
                    + "). Existe Tipo de Cambio:(" + existenTiposCambio + ")";
    ...
    public void setActualizacionDepreciacionAcumulada(BigDecimal actualizacionDepreciacionAcumulada) {
    ...
    private BigDecimal calcularDepreciacionMensual(LocalDate fechaCalculo, BigDecimal factorActualizacion) {
        ...
        if (!deprecia) {
            ...
            // ***(14)	= (12) * (11) * (10) /365
            BigDecimal diasConsumidos = new BigDecimal(ChronoUnit.DAYS.between(fechaActivo, fechaCalculo)
                    + DIAS_A_DEPRECIAR_DESDE_REGISTRO); // FIXME Se puede reemplazar por dias consumidos
            depreciacionAFechaCalculo = calcularValorActualizado(factorActualizacion)
                    .multiply(calcularPorcentajeDepreciacion(), MATH_CONTEXT)
                    .multiply(diasConsumidos)
                    .divide(DAYS_ON_YEAR, MATH_CONTEXT);
            ...
            if (fechaActivo.isBefore(fechaPrimeroMesCalculo) || fechaActivo.isEqual(fechaPrimeroMesCalculo)) {
                BigDecimal diasConsumidosPrimeroMes =
                        new BigDecimal(ChronoUnit.DAYS.between(fechaActivo, fechaPrimeroMesCalculo)
                                + DIAS_A_DEPRECIAR_DESDE_REGISTRO); // FIXME Se puede reemplazar por dias consumidos
                depreciacionAPrimeroMesFechaCalculo = calcularValorActualizado(factorActualizacion)
                        .multiply(calcularPorcentajeDepreciacion(), MATH_CONTEXT)
                        .multiply(diasConsumidosPrimeroMes)
                        .divide(DAYS_ON_YEAR, MATH_CONTEXT);
            }
        }
    ...
    private BigDecimal calcularDepreciacionGestion(LocalDate fechaCalculo, BigDecimal factorActualizacion) {
        ...
        if (!deprecia) {
            ...
            BigDecimal valorActualizado =
                    calcularValorActualizado(factorActualizacion).subtract(BigDecimal.ONE, MATH_CONTEXT);
            if (fechaCalculo.isBefore(fechaVidaUtil)) {
                diasConsumidos = (int) (ChronoUnit.DAYS.between(fechaActivo, fechaCalculo)
                        + DIAS_A_DEPRECIAR_DESDE_REGISTRO); // FIXME Se puede reemplazar por dias consumidos
                valorActualizado = calcularValorActualizadoCosto(factorActualizacion)
                        .multiply(calcularPorcentajeDepreciacion(), MATH_CONTEXT)
                        .multiply(new BigDecimal(diasConsumidos), MATH_CONTEXT)
                        .divide(DAYS_ON_YEAR, MATH_CONTEXT);
            }
            // Si el calculo se lo hace de un activo adquirido diferente al periodo actual
            if (fechaActivo.getYear() <= (fechaCalculo.getYear() - 1)) {
                ...
                temp2 = calcularValorActualizadoCosto(factorActualizacion)
                        .multiply(calcularPorcentajeDepreciacion(), MATH_CONTEXT)
                        .multiply(new BigDecimal(diasConsumidos), MATH_CONTEXT)
                        .divide(DAYS_ON_YEAR, MATH_CONTEXT);
            }
    ...
    private BigDecimal calcularDepreciacionAcumulada(LocalDate fechaCalculo, BigDecimal factorActualizacion) {
        ...
        if (deprecia) {
            ...
            if (fechaCalculo.isAfter(fechaVidaUtil) || fechaCalculo.isEqual(fechaVidaUtil)) {
                ...
                // FIXME Se puede cambiar por la función diasConsumidos =  ChronoUnit.DAYS.between(fechaCalculo,
                // fechaActivo).getDays() + 1
                BigDecimal valorDepreciacion;
                BigDecimal diasConsumidos = new BigDecimal(
                        ChronoUnit.DAYS.between(fechaActivo, fechaCalculo) + DIAS_A_DEPRECIAR_DESDE_REGISTRO);
                BigDecimal valorActualizadoCosto = calcularValorActualizadoCosto(factorActualizacion);
                valorDepreciacion = valorActualizadoCosto
                        .multiply(calcularPorcentajeDepreciacion())
                        .multiply(diasConsumidos)
                        .divide(DAYS_ON_YEAR, MATH_CONTEXT);
                if (valorDepreciacion.compareTo(valorActualizadoCosto) >= 0) {
                    result = valorActualizado;
                } else {
                    result = valorDepreciacion.add(
                            calcularValorActualizadoDepreciacionAcumulada(factorActualizacion), MATH_CONTEXT);
                }
            }
        }
    ...
    private BigDecimal calcularValorActualizadoCosto(BigDecimal factorActualizacion) {
        if (!actualiza) {
            ...
            if (incorporacionEspecial) {
                ...
                return costoInicial
                        .subtract(depreciacionAcumuladaInicial, MATH_CONTEXT)
                        .multiply(factorActualizacion, MATH_CONTEXT);
            } else {
    ...
    private BigDecimal calularFactor(
            LocalDate fechaCalculo, BigDecimal tipoCambioCalculo, BigDecimal tipoCambioVidaUtil) {
        if (!deprecia) {
            if (!actualiza) {
                ...
                } else { // Solo cuando la fechaActivo == fechaCalculo // FIXME se puede omitir el if/else si se valida
                    // que las fechas de calculo sean antes de la fecha de adquisicion
                    return BigDecimal.ONE;
                }
            }
        } else {
    ...
    public BigDecimal getDiasVidaUtilResidualNominal() {
        if (deprecia
                && factorDepreciacion.compareTo(BigDecimal.ZERO) > 0
                && costoInicial.compareTo(BigDecimal.ZERO) > 0) {
            BigDecimal dias = BigDecimal.ONE
                    .divide(factorDepreciacion, MATH_CONTEXT)
                    .subtract(
                            depreciacionAcumuladaInicial.divide(
                                    costoInicial.multiply(factorDepreciacion, MATH_CONTEXT), MATH_CONTEXT),
                            MATH_CONTEXT);
    ...
    public void setDepreciacionAcumuladaInicial(BigDecimal depreciacionAcumuladaInicial) {
