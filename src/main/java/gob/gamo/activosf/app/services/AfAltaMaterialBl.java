...
public class AfAltaMaterialBl {
    ...
    public void registroAfAltaMaterial(AfAltaMaterial afAltaMaterial, List<AfAltaMaterialDetalle> afAltaMaterialDetalleList, 
    		boolean tieneFactura, String numeroFactura, Date fechaFactura, UserRequestVo userRequestVo) {
    	TxTransaccion txTransaccion = txTransaccionBl.generateTxTransaccion(userRequestVo);
    ...
    public void actualizarAfAltaMaterial(AfAltaMaterial afAltaMaterial, List<AfAltaMaterialDetalle> afAltaMaterialDetalleList, 
    		boolean tieneFactura, String numeroFactura, Date fechaFactura, UserRequestVo userRequestVo) {
    	TxTransaccion txTransaccion = txTransaccionBl.generateTxTransaccion(userRequestVo);
    ...
    public void ingresarAfAltaMaterial(AfAltaMaterial afAltaMaterialIn, UserRequestVo userRequestVo) {
    	...
    	for( AfAltaMaterialDetalle afAltaMaterialDetalle: afAltaMaterial.getAfAltaMaterialDetalleList()) {
	    	AfKardexMaterial afKardexMaterial = 
	    			afKardexMaterialBl.getAfKardexMaterialByGestionAndMaterialAndAlmacen(afAltaMaterial.getGestion(),
	    					afAltaMaterialDetalle.getIdMaterial(), afAltaMaterial.getIdAlmacen(),false);
	    	if(afKardexMaterial == null) {
	    		afKardexMaterial = afKardexMaterialBl.crearAfKardexMaterial(afAltaMaterial.getGestion(),
    					afAltaMaterialDetalle.getIdMaterial(), afAltaMaterial.getIdAlmacen(), userRequestVo);
	    	}
	    	...
	    	String detalle = "Alta de material a almacen " + afAltaMaterial.getIdAlmacen().getNombre() + ". ";
	    	...
	    	afKardexMaterial.setSaldoCantidad(afKardexMaterial.getSaldoCantidad() + afAltaMaterialDetalle.getCantidad());
	    	if (afAltaMaterial.getIdAlmacen().getEsValorado()) {
	    		BigDecimal importeTotal = afAltaMaterialDetalle.getImporteUnitario().multiply(new BigDecimal(afAltaMaterialDetalle.getCantidad())); 
	    		afKardexMaterial.setSaldoImporte(afKardexMaterial.getSaldoImporte().add(importeTotal));
	    	}
